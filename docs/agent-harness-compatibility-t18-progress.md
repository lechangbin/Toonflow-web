...
2026-09-26 跨阶段预验收补强：旧单资产图片测试还在寻找已移除的 `handleGenerate`，暴露新计费审批面板在可能阻塞的 Vendor POST 期间因 `busy` 停止轮询。现在提交前立即读取权威请求状态，并在提交仍在途时保持 5 秒轮询；旧直接生成入口的测试改为约束受控面板接线。修复从 Web T09 逐级合并到 T18。T18 合并时将两套同名 Trace 类型与测试合并：保留 T18 的 Project/Run 与视频 ID 安全校验，也保留因果链、失败分类和保留策略字段。相关定向测试 19/19，通过全量 Web Node 测试 125/125。工作树 `node_modules` 指向原仓库时曾有 5 个 TS2742 路径错误；T18 在 TypeScript 配置加入 `preserveSymlinks` 后，标准 `yarn type-check` 通过。完整 `yarn build` 首轮发现共享 Trace 抽屉的生成组件声明未同步，提交自动生成的 `src/types/components.d.ts` 后重跑通过。最终 `dist/index.html` SHA-256 仍为 `132c528f8113d07a0c744a06665ae0b54abce0d4e3e36b224636138f02316437`，与 App 阶段 bundle 一致。这不代表 T18 浏览器、恢复与旧 Socket 迁移全部完成。
