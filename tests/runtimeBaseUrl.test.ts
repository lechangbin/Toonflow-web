...
const remoteLocation = { protocol: "http:", origin: "http://127.0.0.1:10589" };

test("web deployments use their current origin for API requests", () => {
  assert.equal(getInitialApiBaseUrl(remoteLocation), "http://127.0.0.1:10589/api");
});

test("the persisted legacy localhost default migrates without replacing custom endpoints", () => {
  assert.equal(migrateLegacyWebApiBaseUrl(legacyLocalApiBaseUrl, remoteLocation), "http://127.0.0.1:10589/api");
