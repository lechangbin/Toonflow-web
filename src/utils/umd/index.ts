...
export async function loadUmdNode(pluginNodeId: `${string}:${string}`, force = false) {
  ...
  const plugin = manifestList.value.find((m) => nodeId.startsWith(`${m.id}:`));
  if (!plugin) throw new Error(`Plugin ${pluginId} not found`);
  const nodeInfo = plugin.nodes[nodeId.split(":")[1]];
  if (!nodeInfo) throw new Error(`Node ${nodeId} not found in plugin ${pluginId}`);
  const Comp = await loadUmd(`${plugin.url}/${nodeInfo.path}`, nodeInfo.name);
