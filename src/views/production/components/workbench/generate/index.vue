<template>
  <div class="index fc">
    <imageSelect
      v-model="imageList"
      :capability="modelParmas.capability"
    ...
    <div class="generate ac">
      <t-card v-if="currentTrack" :title="'#' + (activeTrackIndex + 1) + $t('workbench.generate.generateText')" header-bordered class="prompt">
        <template #actions>
          <t-button size="small" :loading="currentTrack.state === '生成中'" @click="genText">{{ $t("workbench.generate.generateText") }}</t-button>
        </template>
        <div @focusout="handlePromptBlur">
    ...
    <newTrack
      ...
      @change="trackChange"
      @get-data="getGenerateData"
      @batch-prompt="batchGenText"
      @batch-video="batchGenVideo" />
  </div>
</template>
...
import {
  ...
  type VideoWorkbenchTrackProjection,
} from "@/videoContract";
...
function configureSelection(modelSelection: string, capabilityId?: VideoCapabilityId | null, output?: Partial<VideoOutputSelection> | null) {
  ...
  const aspectRatio = preset.aspectRatios.includes(output?.aspectRatio as VideoAspectRatio)
    ? (output!.aspectRatio as VideoAspectRatio)
...
function selectedVideoConfiguration(track = currentTrack.value): VideoSelection {
  ...
  if (!isCurrentTrack && track?.vendorId && track.modelId && track.capabilityId) {
    ...
    const aspectRatio = preset.aspectRatios.includes(track.outputSelection?.aspectRatio as VideoAspectRatio)
      ...
      : preset.aspectRatios.includes(project.value?.videoRatio as VideoAspectRatio)
        ...
        : preset.aspectRatios[0];
    const duration = durations.includes(track.outputSelection?.duration ?? -1)
      ? track.outputSelection!.duration
      : durations.includes(track.duration)
        ? track.duration
        : durations[0];
  ...
  return createVideoSelection(
    modelParmas.value.modelSelection,
    modelParmas.value.model,
    modelParmas.value.capability.id,
    modelParmas.value.output.presetId,
    modelParmas.value.output.aspectRatio,
    track?.duration || modelParmas.value.output.duration,
    modelParmas.value.audio.generation === "native" && modelParmas.value.audio.enabled,
  );
}
...
function sortByRole(items: UploadItem[]): UploadItem[] {
  ...
  return [...items]
    .filter((item) => item.inputRole && roles.includes(item.inputRole))
    .sort((a, b) => roles.indexOf(a.inputRole!) - roles.indexOf(b.inputRole!));
}
const imageList = computed({
  get(): UploadItem[] {
    const track = currentTrack.value;
    if (!track) return [];
    const cached = project.value && getCache(project.value.id, episodesId.value, track.id);
    return sortByRole((cached?.length ? cached : (track.medias as UploadItem[])) ?? []);
  },
  set(value: UploadItem[]) {
    const track = currentTrack.value;
    if (!track || !project.value) return;
    track.medias = sortByRole(value) as TrackMedia[];
...
function hydrateInputRoles(track: VideoWorkbenchTrackProjection): TrackItem {
  return hydrateVideoWorkbenchTrackProjection(track) as TrackItem;
}
...
async function getGenerateData() {
  ...
  const workbench = data as VideoWorkbenchData;
  const defaults = workbench.projectDefaults;
  ...
  storyboardList.value = (workbench.storyboardList ?? []) as StoryboardItem[];
  const tracks = (workbench.trackList ?? []).map(hydrateInputRoles);
  ...
  for (const track of tracks) {
    const cached = getCache(project.value.id, episodesId.value, track.id);
    if (cached?.some((item) => item.inputRole)) track.medias = cached as TrackMedia[];
    lastSavedPrompt.set(track.id, track.prompt ?? "");
  }
...
function syncTrackSelection() {
  const track = currentTrack.value;
  const vendorId = track?.vendorId;
  const modelId = track?.modelId;
  if (!vendorId || !modelId) return;
  configureSelection(`${vendorId}:${modelId}`, track.capabilityId, track.outputSelection ?? undefined);
  if (track?.audioSelection) modelParmas.value.audio = track.audioSelection;
}

function promptSubject(track: TrackItem): string {
  return (
    track.prompt?.trim() ||
...
async function handlePromptBlur() {
  ...
  try {
    ...
    const request = buildUpdatePromptRequest({
      ...
      trackId: track.id,
      selection,
      medias: imageList.value,
      renderedPrompt: track.prompt,
    });
    const { data } = await axios.post("/production/workbench/updateVideoPrompt", request);
    applyTrackOwnedSelection(track, selection, request.inputs);
    track.promptRevisionId = data.promptRevisionId;
    lastSavedPrompt.set(track.id, data.renderedPrompt);
  } catch (error) {
    window.$message.error((error as Error)?.message ?? "保存提示词失败");
  }
}

function applyTrackOwnedSelection(
  track: TrackItem,
  selection: VideoSelection,
  inputReferences: ReturnType<typeof buildSemanticInputReferences>,
) {
  track.vendorId = selection.vendorId;
  track.modelId = selection.modelId;
  track.capabilityId = selection.capabilityId;
  track.inputReferences = inputReferences;
  track.outputSelection = selection.output;
  track.audioSelection = selection.audio;
  track.duration = selection.output.duration;
}

function applyPromptRevision(
  track: TrackItem,
  revision: PromptRevisionResponse,
  selection: VideoSelection,
  inputReferences: ReturnType<typeof buildSemanticInputReferences>,
) {
  track.prompt = revision.renderedPrompt;
  track.promptRevisionId = revision.promptRevisionId;
  applyTrackOwnedSelection(track, selection, inputReferences);
  track.state = "已完成";
  lastSavedPrompt.set(track.id, revision.renderedPrompt);
}
async function genText() {
  const track = currentTrack.value;
  if (!track || track.state === "生成中") return;
  try {
    track.state = "生成中";
    const selection = selectedVideoConfiguration(track);
    const request = buildPromptRequest({
      ...
      selection,
      medias: imageList.value,
      subject: promptSubject(track),
    });
    const { data } = await axios.post("/production/workbench/generateVideoPrompt", request);
    applyPromptRevision(track, data, selection, request.inputs);
  } catch (error) {
    track.state = "生成失败";
...
async function batchGenText(trackIds: number[]) {
  const tracks = trackList.value.filter((track) => trackIds.includes(track.id));
  if (!tracks.length) return;
  try {
    tracks.forEach((track) => (track.state = "生成中"));
    const items = tracks.map((track) =>
      buildPromptRequest({
        projectId: Number(project.value!.id),
        trackId: track.id,
        selection: selectedVideoConfiguration(track),
        medias: track.medias,
        subject: promptSubject(track),
      }),
    );
    await axios.post("/production/workbench/batchGeneratePrompt", { items, concurrentCount: otherSetting.value.assetsBatchGenereateSize });
    await getGenerateData();
  } catch (error) {
    tracks.forEach((track) => (track.state = "生成失败"));
    window.$message.error((error as Error)?.message ?? "批量生成提示词失败");
  }
}
...
function attachStartedTasks(response: unknown) {
  for (const task of normalizeStartedTasks(response)) {
    const track = trackList.value.find((item) => item.id === task.trackId);
    if (track) track.videoList.push({ id: task.videoId, state: "生成中", src: "", artifactRevisionId: task.artifactRevisionId });
  }
}
async function startGeneration(items: ReturnType<typeof generationItem>[], batch: boolean) {
  const endpoint = batch ? "/production/workbench/batchGenerateVideo" : "/production/workbench/generateVideo";
  const payload = {
    projectId: Number(project.value!.id),
    scriptId: episodesId.value,
    requestedBy: "user",
    ...(batch ? { items } : { item: items[0] }),
  };
  const { data } = await axios.post(endpoint, payload);
  attachStartedTasks(data);
...
function generateVideo() {
  const dialog = DialogPlugin.confirm({
    ...
    onConfirm: async () => {
      dialog.destroy();
      try {
        await startGeneration([generationItem(currentTrack.value)], false);
      } catch (error) {
        window.$message.error((error as Error)?.message ?? "视频发起生成请求失败");
      }
    },
    onCancel: () => dialog.destroy(),
  });
}
async function batchGenVideo(trackIds: number[]) {
  try {
    await startGeneration(trackList.value.filter((track) => trackIds.includes(track.id)).map(generationItem), true);
  } catch (error) {
    window.$message.error((error as Error)?.message ?? "批量生成视频失败");
  }
}
function trackChange(previousIndex?: number) {
  const previous = previousIndex == null ? undefined : trackList.value[previousIndex];
  if (previous && project.value) setCache(project.value.id, episodesId.value, previous.id, previous.medias);
  syncTrackSelection();
}

const pendingVideoIds = computed(() =>
  trackList.value.flatMap((track) => track.videoList.filter((video) => video.state === "生成中").map((video) => video.id)),
);
let pollTimer: ReturnType<typeof setInterval> | null = null;
async function pollVideos() {
  if (!pendingVideoIds.value.length || !project.value) return;
  const { data } = await axios.post("/production/workbench/checkVideoStateList", {
    projectId: Number(project.value.id),
    scriptId: episodesId.value,
    videoIds: pendingVideoIds.value,
  });
  for (const update of data ?? [])
    for (const track of trackList.value) {
      const video = track.videoList.find((item) => item.id === update.id);
      if (video) Object.assign(video, { ...update, state: update.state === "生成成功" ? "已完成" : update.state });
    }
}
watch(
  pendingVideoIds,
  (ids) => {
    if (ids.length && !pollTimer) pollTimer = setInterval(pollVideos, 3000);
    if (!ids.length && pollTimer) {
...
onMounted(async () => {
  try {
    catalog.value = await getVideoCapabilityCatalog();
    if (project.value?.videoVendorId && project.value.videoModelId)
      configureSelection(`${project.value.videoVendorId}:${project.value.videoModelId}`, project.value.videoCapabilityId);
    await getGenerateData();
  } catch (error) {
    window.$message.error((error as Error)?.message ?? "加载视频工作台失败");
  }
});
onUnmounted(() => {
