<template>
  <div class="generateContainer">
    ...
    <div class="shotListArea">
      ...
      <div class="shotListWrapper f" ref="trackListRef">
        <div class="shotList">
          <div
            ...
            <div class="shotImageWrapper">
              ...
              <div v-if="isManualSlot(item)" class="shotRemoveBtn" @click.stop="removeSpawnSlotFn(item.id)">
                <i-close size="12" fill="#fff" />
              </div>
            </div>
          </div>
        </div>
        <div class="addSpawnSlot ac c" @click="addSpawnSlotFn">
          <i-plus theme="filled" size="24" fill="#999999" />
        </div>
      </div>
    </div>
  ...
  return axios
    ...
    .then(({ data }) => {
      ...
      const list: StoryboardItem[] = videoList.map((vItem) => {
        ...
        // 保留当前 shotList 中用户手动选中的 selectedVideoId，避免轮询刷新后丢失
        const existing = shotList.value.find((s) => s.id === vItem.id);
        return {
          id: vItem.id,
          prompt: vItem.prompt,
          ...
          selectedVideoId: existing?.selectedVideoId ?? undefined,
        };
      });
...
async function handleGenerate() {
  ...
  // 生成视频后自动在视频轨道末尾添加一个空槽位，方便后期放入数据生成视频
  const newSlot = {
    id: Date.now(),
    prompt: "",
    filePath: "",
    duration: selectedDuration.value || 4,
  };
  flowData.value.workbench.videoList.push(newSlot);
  // 将新槽位同步到 shotList 中展示
  shotList.value.push({
    id: newSlot.id,
    prompt: newSlot.prompt,
    filePath: newSlot.filePath,
  ...
  if (trackListRef.value) {
    trackListRef.value.scrollLeft = trackListRef.value.scrollWidth;
  }
}
...
/** 判断是否为手动添加的空槽位（无视频记录、无配置、无图片的槽位可移除） */
function isManualSlot(item: StoryboardItem): boolean {
  return !item.filePath && !item.config && (!item.videos || item.videos.length === 0);
}

/** 移除手动添加的槽位 */
function removeSpawnSlotFn(id: number | string) {
  // 从 flowData 中移除
  const flowIdx = flowData.value.workbench.videoList.findIndex((v) => v.id === id);
  if (flowIdx !== -1) {
    flowData.value.workbench.videoList.splice(flowIdx, 1);
  }
  // 从 shotList 中移除
  const shotIdx = shotList.value.findIndex((s) => s.id === id);
  if (shotIdx !== -1) {
    shotList.value.splice(shotIdx, 1);
  }
  // 从勾选集合中移除
  checkedIds.value.delete(id);
  // 如果移除的是当前选中的分镜，清空选中状态或选中第一个
  if (currentShot.value?.id === id) {
    if (shotList.value.length > 0) {
      selectShot(shotList.value[0].id);
    } else {
